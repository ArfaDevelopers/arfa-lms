          ...
          <tbody>
            @forelse ($topics as $topic)
              <tr>
                ...
                <td width="90px">
                  <form action="{{ route('topics.destroy', $topic->id) }}" method="POST" id="delete-form-{{ $topic->id }}">
                    @csrf
                    @method("DELETE")
                    <a href="{{ route('topics.edit', $topic->id) }}" class="btn btn-outline-primary btn-sm">
                      <i class="fas fa-edit"></i>
                    </a>
                    <button type="button" class="btn btn-outline-danger btn-sm" onclick="confirmDelete({{ $topic->id }})">
